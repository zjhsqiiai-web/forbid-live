...
class ForbidToken(discord.Client):
    ...
    "# █▓▒░ 👑 FORBID KING ║ ➔ 👿 **{user_text} GULAMI KR** ⪧ 【🔱】"
        ...
        try:
            ...
            print(f"[{self.user.name}] Engaging infiltration protocol. Stagger: {stagger:.2f}s...")
            ...
            async def join_server():
                ...
                try:
                    ...
                    await invite.accept()
                    print(f"✅ [{self.user.name}] Successfully joined {invite_code}")
                    ...
                    print(f"❌ [{self.user.name}] Join failed: {e}")
        ...
        try:
            ...
            print(f"[{self.user.name}] Engaging extraction protocol. Stagger: {stagger:.2f}s...")
            ...
            async def leave_server():
                ...
                try:
                    ...
                    if guild_to_leave:
                        ...
                        print(f"✅ [{self.user.name}] Successfully extracted from {guild_to_leave.name}")
                        
                        # Clean chat output
                        if is_targeted or (self.user.id % 8 == 0 or self.user.id % 8 == 1):
                            await message.channel.send(f"✅ FORB1D🔥 **{self.user.name}** extracted from: `{guild_to_leave.name}`")
                    else:
                        print(f"⚠️ [{self.user.name}] Aborted: Not in network {invite_code}")
                        if is_targeted or self.user.id % 8 == 0:
                            await message.channel.send(f"⚠️ **{self.user.name}** is not in that network.")
                            
                except Exception as e:
                    print(f"❌ [{self.user.name}] Extraction failed: {e}")
                    if is_targeted or self.user.id % 8 == 0:
            ...
            async def gcnc_loop():
                ...
                while True:
                    try:
                        ...
                        print(f"🔄 [{self.user.name}] Flashed GC name: {new_gc_name}")
        ...
        if message.channel.id in gcnc_tasks and gcnc_tasks[message.channel.id]:
            ...
            if self.user.id % 8 == 0:
                ...
                print(f"🛑 [{self.user.name}] Stopped gcnc tasks in GC: {message.channel.id}")
        else:
            if self.user.id % 8 == 0:
